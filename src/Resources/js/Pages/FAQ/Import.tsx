...
export default function Import() {
  ...
  const handleFileUpload = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(route('faq.import'), {
        method: 'POST',
        body: formData,
        headers: {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        window.location.href = route('support-ticket-faq.index');
      } else {
  ...
  const downloadSample = () => {
    const csvContent = "title,description\nHow to create account?,You can create account by clicking sign up button,What is FAQ?,FAQ stands for Frequently Asked Questions";
    const blob = new Blob([csvContent], { type: 'text/csv' });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'sample_faq.csv';
    a.click();
  ...
  return (
    <AuthenticatedLayout
      breadcrumbs={[
        { label: t('Support Tickets'), href: route('support-ticket-faq.index') },
        { label: t('FAQ'), href: route('support-ticket-faq.index') },
        { label: t('Import') }
      ]}
      pageTitle={t('Import FAQ')}
    >
      <Head title={t('Import FAQ')} />

      <Card>
        <CardHeader>
          <CardTitle>{t('Import FAQ')}</CardTitle>
        </CardHeader>
        <CardContent>
          {!showMapping ? (
            <form onSubmit={handleFileUpload} className="space-y-4">
              <div>
                <Label className="text-red-600">{t('Download Sample FAQ CSV File')}</Label>
                <Button 
                  type="button" 
                  variant="outline" 
                  size="sm" 
